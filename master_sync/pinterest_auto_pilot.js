...
// Manually load env variables from .env in the root if it exists
let PINTEREST_TOKEN = process.env.PINTEREST_ACCESS_TOKEN;
if (!PINTEREST_TOKEN) {
    try {
        ...
        if (fs.existsSync(envPath)) {
            ...
            if (match && match[1]) {
                PINTEREST_TOKEN = match[1].trim();
            }
        }
    } catch (e) {
        console.error("[PINTEREST] Failed to load .env file:", e.message);
    }
}
